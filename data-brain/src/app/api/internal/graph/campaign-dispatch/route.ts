...
import { authorizeGraphWorkerBearerRequest, executeConfiguredColdCampaignTick } from '@/lib/graph-runtime';

export const runtime = 'nodejs';

export async function POST(request: Request) {
  const headers = { 'Cache-Control': 'private, no-store, max-age=0' };
  if (!authorizeGraphWorkerBearerRequest(request)) {
  ...
  try {
    const result = await executeConfiguredColdCampaignTick();
    const status = result.state === 'ambiguous_halted' ? 503
      : result.state === 'off' || result.state === 'deferred' ? 409 : 200;
