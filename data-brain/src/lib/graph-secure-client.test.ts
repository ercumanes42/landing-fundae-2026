...
test('Graph reads honor Retry-After without retrying early', async () => {
  ...
  let calls = 0;
  const client = new SecureMicrosoftGraphClient({
    mailboxUserId: 'mailbox@example.com', mailboxAddress: 'mailbox@example.com', accessToken: token,
...
test('Graph read defers when Retry-After exceeds the worker budget', async () => {
  let calls = 0;
  const client = new SecureMicrosoftGraphClient({
    mailboxUserId: 'mailbox@example.com', mailboxAddress: 'mailbox@example.com', accessToken: token, maxRetryDelayMs: 1_000,
    fetchImpl: (async () => {
      calls += 1;
...
test('draft integrity requests and validates optional mailbox identity fields', async () => {
  let requestedUrl = '';
  const client = new SecureMicrosoftGraphClient({
    mailboxUserId: 'mailbox@example.com',
    mailboxAddress: 'mailbox@example.com',
    ...
    fetchImpl: (async (url) => {
      ...
      return Response.json({
        ...
        toRecipients: [{ emailAddress: { address: 'pilot@example.com' } }],
        from: { emailAddress: { address: 'mailbox@example.com' } },
        sender: { emailAddress: { address: 'mailbox@example.com' } },
        replyTo: [{ emailAddress: { address: 'mailbox@example.com' } }],
  ...
  const integrity = await client.getDraftIntegrity('ImmutableDraft');
  assert.equal(integrity?.from, 'mailbox@example.com');
  assert.equal(integrity?.sender, 'mailbox@example.com');
  assert.deepEqual(integrity?.replyTo, ['mailbox@example.com']);
  assert.match(requestedUrl, /from%2Csender%2CreplyTo/);
  assert.match(requestedUrl, /singleValueExtendedProperties/);
});
...
test('Graph draft uses a GUID user path while asserting the separate mailbox address', async () => {
  const mailboxUserId = 'aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee';
  let requestedUrl = '';
  ...
  const client = new SecureMicrosoftGraphClient({
    ...
    fetchImpl: (async (url, init) => {
      ...
      return Response.json({
        id: 'ImmutableDraft', changeKey: 'Change1', isDraft: true,
  ...
  await client.createDraft({
    recipient: 'pilot@example.com', subject: 'subject', htmlBody: '<p>body</p>',
    marker: 'e'.repeat(64), attachments: [],
  });
  assert.match(requestedUrl, new RegExp(`/users/${mailboxUserId}/messages$`));
  const identity = { emailAddress: { address: 'mailbox@example.com' } };
  assert.deepEqual(body.from, identity);
  assert.deepEqual(body.sender, identity);
  assert.deepEqual(body.replyTo, [identity]);
});
