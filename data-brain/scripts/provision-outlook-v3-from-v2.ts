...
try {
  const apply = parseTransactionalOutlookV3CloneArgs(process.argv.slice(2));
  const summary = await cloneTransactionalOutlookV3FromV2(apply, {
    async findSource() {
    ...
    async findTargetExisting(rows) {
      ...
      return selectRows<TransactionalPilotLeadRow>(
        'leads',
        `select=${LEAD_SELECT}&${filter}&limit=4`,
      );
    },
